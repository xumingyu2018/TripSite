...
导入main.js中`import "normalize.css"`
...
[Vant官方网址](https://vant-contrib.gitee.io/vant/#/zh-CN)
...
```
4. **方式四**：F12直接查找对应的子组件选择器，直接修改CSS（非自定义插槽）。
    - `:deep(子组件中元素选择器)`进行修改
...
配置路由中需要隐藏TabBar一个`meta`属性，在父组件调用`<TabBar>`时进行`v-if`判断（使用`useRoute`获取当前活跃路由）
...
引入自定义封装的轮播组件到父组件进行调用，导航栏使用Vant中的`<van-nav-bar>`，并监听返回按钮点击（这里未使用pinia进行状态管理）
